function data = readOEphys(dataDir, dataClass, raw_data_folder, dataRange, dataType)
%{
...
dirFiles = dirFiles(~contains({dirFiles.name},{'._'})); 
...
switch dataClass
    ...
    case 'events'
        if (nargin > 3) && ~isempty(dataRange), warning('event data do not support channel range and data range'); end
        ...
        elseif strcmp(dataEngine, 'rawbin') 
            ...
            if line_txt == -1 % file is empty for some reason
                %filepath = fullfile(dataDir);
                cntData = readOEphys(dataDir, 'continuous', raw_data_folder, [1 1; 1 10]);
